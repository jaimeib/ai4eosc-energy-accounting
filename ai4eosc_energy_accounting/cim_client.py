...
def get_token(
    ...
    session = session or http_mod.session_with_retries()
...
def push_records(
    ...
    session = http_mod.session_with_retries()
    publish_url = f"{base_url.rstrip('/')}/gd-cim-api/v1/submit"
    token = get_token(
        base_url, email, password, verify_ssl=verify_ssl, timeout=timeout, session=session
    )
    headers = {
