...
5. Sends the batch via `sender.type`:
   ...
   - `"file"`: appends the same JSON payload to `sender.file_path`
     ...
     CIM.
6. Before sending, checks which allocations the *previous* run reported as
...
remembers it as open; the *next* run then closes it via step 6 once it
...
boundary coincidence *and* a full-window data outage) and never affects
...
the window edge" case from *Known limitation*), the `datacenter` → `Owner`
